#!/usr/bin/env python3
"""
train_mumu_bpm_lora.py
=====================
Fine-tunes MuMu-LLaMA on BPM extraction using its native
...
DATASET_JSONL = "/data/mg546924/llm_beatmap_generator/bpm_sft_dataset/bpm_dataset.jsonl"
OUTPUT_DIR = "/data/mg546924/models/mumu-llama-lora-bpm"
...
def main():
    ...
    class BPMDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            # Parse the messages format used by Qwen dataset
            audio_path = item["messages"][0]["content"][0]["audio_url"]
            if "/Users/mukeshguntumadugu/" in audio_path:
                audio_path = audio_path.replace("/Users/mukeshguntumadugu/", "/data/mg546924/")
            prompt = item["messages"][0]["content"][1]["text"]
            response = str(item["messages"][1]["content"][0]["text"])
    ...
    print("Loading BPM dataset...")
    full_dataset = BPMDataset(DATASET_JSONL, tokenizer, MAX_WORDS)
    ...
    loss_log = []
    print(f"\n{'='*60}")
    print(f"Starting MuMu-LLaMA BPM Training: {NUM_EPOCHS} Epochs")
    print(f"{'='*60}")
    ...
    print(f"\n✅ MuMu-LLaMA BPM Training Complete! Final model saved to {OUTPUT_DIR}")
