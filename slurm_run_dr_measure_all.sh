#!/bin/bash
#SBATCH --job-name=dr_batch
#SBATCH --nodes=1
#SBATCH --gpus-per-node=1
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --time=160:00:00
#SBATCH --output=logs/dr_batch_%j.out

echo "=== DEEPRESONANCE MEASURE-BY-MEASURE: Starting 20 Loop Iterations === $(date)"
...
PYTHON_DRIVER="/data/mg546924/conda_envs/deepresonance_env/bin/python"
...
for ITERATION in {1..20}; do
    ...
    find "$BASE_DIR" -mindepth 1 -maxdepth 1 -type d | sort | while read -r SONG_DIR; do
        ...
        $PYTHON_DRIVER -u scripts/DeepResonance/deepresonance_measure_generator.py
    done
done
