...
for ext in formats:
    print(f"Generating pancaked graph for {ext.upper()}...")
    ...
    # Plot each group
    # We use a lower alpha to see overlapping points if they exist
    for label, data in color_groups.items():
        if data['x']:
            plt.scatter(data['x'], data['y'], c=data['color'], s=data['s'], label=label, alpha=0.3, edgecolors='black', linewidth=0.5)
    ...
    plt.title(f'Librosa {ext.upper()} All 10 Runs Pancaked - RMSE vs. Actual BPM', fontsize=24)
    ...
    plt.tight_layout()
    out_name = f'Librosa_Pancaked_{ext.upper()}.png'
