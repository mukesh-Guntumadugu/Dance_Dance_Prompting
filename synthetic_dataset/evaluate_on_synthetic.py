...
DATASET_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "synthetic_data")
...
def main():
    ...
    if not os.path.exists(DATASET_DIR):
        ...
        print("Please run build_synthetic_dataset.sh first!")
        return
        
    for category_dir in os.listdir(DATASET_DIR):
        cat_full_dir = os.path.join(DATASET_DIR, category_dir)
        if not os.path.isdir(cat_full_dir):
            continue
            
        for d in os.listdir(cat_full_dir):
            full_dir = os.path.join(cat_full_dir, d)
            if os.path.isdir(full_dir):
                ...
                if os.path.exists(audio_path) and os.path.exists(json_path):
                    # Sort numerically by parsing song_XX
                    try:
                        num = int(d.split('_')[1])
                    except:
                        num = 999
